...
class StrategicEnv(gym.Env):
    ...
    def _adventure_wait(self, timeout=300):
        ...
        while remaining > 0:
            try:
                result = self._libml.adventure_wait_for_turn()
                if result >= 0:
                    self.logger.debug(f"yourTurn: player={result} ({time.time()-t0:.1f}s)")
                    return
                else:
                    self.logger.debug(f"adventure_wait_for_turn returned {result}, retrying")
                # -1: retry
            except Exception as e:
                self.logger.debug(f"adventure_wait retrying: {e}")
    ...
    def _send_action(self, action: int):
        ...
        if self._libml is not None:
            self._libml.adventure_send_action(action)
